...
int main(int argc, char* argv[]) {
  ...
  for(Long64_t i = 0; i < nEntries; ++i) {
    ...
    if (  ( !isSolid && silicon_mult_first == 1 && silicon_mult_second == 1 ) || ( isSolid && silicon_mult_first ==1 ) ) { 
	...
	else vertex.SetXYZ (0.,0.,( targetForwardZ + targetBackwardZ ) /2. ); //set vertex to middle of the target in case of solid target
	if(verbose) {
	...
	zErrorVsThetaRec->Fill(recoilThetaRec,  vertex.Z()-reactionZSim );
	zErrorVsThetaSim->Fill(recoilThetaSim,  vertex.Z()-reactionZSim );
